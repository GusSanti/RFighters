...
local function InitializeFightingHUD()
...
local function InitializeScreenTransition()
	ScreenTransition.Init(Blackout)
end

-- LocalQueue connects match server events at require time, before async UI system
-- Init tasks necessarily finish. Prime dependent submodules immediately.
InitializeScreenTransition()
InitializeFightingHUD()

-- UI.Init connects buttons before async system Init tasks finish, so map selection
-- state must be ready as soon as LocalQueue is required.
...
local fadeWatchdogToken = 0
local FADE_WATCHDOG_TIMEOUT = 20
...
-- O servidor manda o ActiveCharacter, que é o id do personagem (ex: "EvilBunny").
-- As ViewportFrames podem estar nomeadas pelo id ou pelo nome de exibição
-- (ex: "Evil Bunny"), então aceitamos os dois para nunca falhar o match.
local function GetCharacterAliases(charName: string): { string }
	local aliases = { charName }
	local lower = charName:lower()
	for _, char in CharacterInfo do
		if char.id:lower() == lower or char.name:lower() == lower then
			table.insert(aliases, char.id)
			table.insert(aliases, char.name)
			break
		end
	end
	return aliases
...
local function MatchesCharacter(vpName: string, aliases: { string }): boolean
	local lower = vpName:lower()
...
MatchSystemEvents.OnClientEvent:Connect(function(action, args)
	...
	elseif action == "EnableFightingFrame" then
		...
		MatchUIInteractionsConnection = MatchUIInteractionsRemote.OnClientEvent:Connect(function(a, ...)
			FightingHUD.HandleEvent(a, ...)
		end)
	...
	elseif action == "FadeIn" then
		...
		-- Watchdog: se nenhum FadeOut chegar (ex: match abortada no servidor porque
		-- o oponente saiu), forca o FadeOut para a tela nao travar no preto.
		fadeWatchdogToken += 1
		local myToken = fadeWatchdogToken
		task.delay(FADE_WATCHDOG_TIMEOUT, function()
			if myToken == fadeWatchdogToken then
				ScreenTransition.FadeOut()
			end
		end)

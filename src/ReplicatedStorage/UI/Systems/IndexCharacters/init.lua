...
function IndexCharacters.ButtonAction(button: GuiButton, action)
	...
	if action == "ClaimReward" then
		...
		local status = GetData:InvokeServer("GetRewardStatus", selectedCharacter)
		if status ~= "Claimable" then
			if status == "Locked" then
				RewardNotificationModule.ShowLockedReward("UNLOCK THIS CHARACTER BEFORE CLAIMING THE REWARD")
			end

...
function module.UseSkill(char: Model)
	...
	local StartAnim  = Animations:FindFirstChild("Ability1")
	...
	task.delay(0.6, function()
		...
		hitbox.OnTouch = function(enemyCharacter)
			...
			if ArenaBounds then
				char:PivotTo(CFrame.new(ArenaBounds:GetPivot().X, ArenaBounds:GetPivot().Y - 38.5, ArenaBounds:GetPivot().Z) * CFrame.Angles(0, math.rad(90), 0))
			end
			...
			Assets.Events.Replicate:FireAllClients("SparrowUltimate", {Char = char, Enemy = enemyCharacter})
			...
			-- Timings originais do Sparrow, agora com applyDamageOrMark
			task.delay(3.37, function()
				applyDamageOrMark(Damage)
			end)
			task.delay(4.01, function()
				applyDamageOrMark(Damage)
			end)
			task.delay(4.17, function()
				applyDamageOrMark(Damage)
			end)
			task.delay(4.32, function()
				applyDamageOrMark(Damage)
			end)
			task.delay(4.48, function()
				applyDamageOrMark(Damage)

				-- Loop de dano a cada 0.1s até 7.17s
				local elapsed = 4.48
				local endTime = 7.17
				...
				connection = RunService.Heartbeat:Connect(function(dt)
					...
					local steps     = math.floor(elapsed / 0.1)
					local lastSteps = math.floor((elapsed - dt) / 0.1)

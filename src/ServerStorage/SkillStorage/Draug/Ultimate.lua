...
function module.UseSkill(char: Model)
	...
	task.delay(0.20, function()
		...
		hitbox.OnTouch = function(enemyCharacter)
			...
			if ArenaBounds then
				char:PivotTo(CFrame.new(ArenaBounds:GetPivot().X, ArenaBounds:GetPivot().Y - 38.3, ArenaBounds:GetPivot().Z) * CFrame.Angles(0, math.rad(90), 0))
			end
			...
			-- SFX exclusivo do Draug (com delay original)
			...
			CutsceneCameraReplicate:FireClient(player, {
				...
				Animation       = Animations.Ability3Camera,  -- nome original do Draug
				CamPartName     = "camera2",
			...
			-- Timings originais do Draug, agora com applyDamageOrMark
			task.delay(3.49, function()
				applyDamageOrMark(Damage)
			end)
			task.delay(8.22, function()
				applyDamageOrMark(Damage)
			end)

			task.delay(8.82, function()
				if not hasDied then

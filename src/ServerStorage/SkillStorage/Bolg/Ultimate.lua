...
function module.UseSkill(char: Model)
	...
	local StartAnim = Animations:FindFirstChild("UltStart")
	...
	task.delay(0.14,function()
		...
		hitbox.OnTouch = function(enemyCharacter)
			...
			if ArenaBounds then
				char:PivotTo(CFrame.new(ArenaBounds:GetPivot().X, ArenaBounds:GetPivot().Y - 37.5, ArenaBounds:GetPivot().Z) * CFrame.Angles(0, math.rad(90), 0))
			end
